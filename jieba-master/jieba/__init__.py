...
re_eng = re.compile('[a-zA-Z0-9]', re.U) #英文含数字

# \u4E00-\u9FD5a-zA-Z0-9+#&\._ : All non-space characters. Will be handled with re_han 没有空格的单词用re_han
...
re_han_cut_all = re.compile("([\u4E00-\u9FD5]+)", re.U) #汉字组
...
class Tokenizer(object):
    ...
    def cut(self, sentence, cut_all=False, HMM=True):
        ...
        if cut_all:
            re_han = re_han_cut_all #一个正则
            re_skip = re_skip_cut_all #一个正则
        else:
        ...
        if cut_all:
            ...
            cut_block = self.__cut_DAG #不是cut_all，如果为HMM，则采用DAG分词，否则采用__cut_DAG_NO_HMM
        else:
    ...
    def load_userdict(self, f):
        ...
        for lineno, ln in enumerate(f, 1):
            ...
            if tag is not None:
                tag = tag.strip()
            self.add_word(word, freq, tag) #将用户词的词频、词性加载到词典中
    ...
    def add_word(self, word, freq=None, tag=None):
        ...
        #如果该词的词频未设置，则
        self.FREQ[word] = freq
        self.total += freq                      #total为所有词总的词频：所有词出现的总次数
        if tag:
            self.user_word_tag_tab[word] = tag #词性加载到用户词词性表
        for ch in xrange(len(word)):
            wfrag = word[:ch + 1]
            if wfrag not in self.FREQ: #将该词内的每个字拆开，如果不在FEREQ中，则加入，置频率为0
                self.FREQ[wfrag] = 0
        if freq == 0:
            finalseg.add_force_split(word) #将该词加入全局词典Force_Split_Words
    ...
    def suggest_freq(self, segment, tune=False):
        ...
        """
        self.check_initialized()
        ftotal = float(self.total) #全局词频
        freq = 1
        if isinstance(segment, string_types): #如果segment为string，则建议将该词不拆开
            word = segment
            for seg in self.cut(word, HMM=False):  #用默认分词模式分词
                freq *= self.FREQ.get(seg, 1) / ftotal #所分子词词频除以全局词频，子词越多，则频率越小
            freq = max(int(freq * self.total) + 1, self.FREQ.get(word, 1)) #最小的频数为1
        else:
        ...
        if tune:
            add_word(word, freq) #如果tune,则将该词添加到FREQ词典中，会不会保存呢？——只放到内存
        return freq
